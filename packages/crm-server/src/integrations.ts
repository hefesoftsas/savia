...
import {
  ...
  validateJsonSchema,
  MAX_DOCUMENT_BYTES,
} from "@savia/crm-shared/openapi";
import { identifier } from "@savia/crm-shared/metadata";
...
export function registerIntegrations(app: Hono<Env>) {
  ...
  app.post("/api/integrations/:id/execute", async (c) => {
    ...
    if (url) await validatePublicDns(url);
