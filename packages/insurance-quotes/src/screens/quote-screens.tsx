...
function ClientMappingPanel({
  ...
  return (
    ...
    >
          ...
          {crmCollections.length > 0 ? (
            ...
            >
              {!crmCollections.some(
                ...
                <option value={mapping.collection}>{mapping.collection}</option>
              ) : null}
      ...
      <div
        className="insurance-mapping-rows"
        role="group"
        aria-label={t("Equivalencia de campos")}
      >
        {rows.map(([collectionField, source]) => (
          <div className="insurance-mapping-row" key={collectionField}>
            <label className="insurance-admin__field">
              <span>
                {t("Campo en")} {mapping.collection || "la colección"}
              </span>
              <input
...
export function InsurancePackageAdminScreen({
  savia,
  showScreenToggles = true,
}: ScreenProps & { showScreenToggles?: boolean }) {
  const t = useInsuranceMessages();
  ...
  const moveProduct = (id: string, direction: -1 | 1) =>
    updateDraft((current) => {
      const ordered = [...current.products].sort(
        ...
        <div className="insurance-admin">
          ...
          >
            ...
            <details className="insurance-admin__products-disclosure">
              <summary>
                {draft.products.filter((product) => product.enabled).length}{" "}
                {t("de")} {draft.products.length} {t("activos")}{" "}
              </summary>
              <div className="insurance-admin__products">
                {[...draft.products]
                  ...
                  .map((product, index) => (
                    <article
                      key={product.id}
                      className="insurance-admin__product"
                    >
                      ...
                      <div className="insurance-admin__product-actions">
                        <button
                          aria-label={t("Subir %{p0}", { p0: product.label })}
                          disabled={index === 0}
                        ...
                        <button
                          aria-label={t("Bajar %{p0}", { p0: product.label })}
          ...
          <section
            className="insurance-admin__panel"
            aria-label={t("Consulta de placa")}
          >
            <div className="insurance-admin__panel-heading">
              <div>
                  ...
                  onChange={(event) =>
                    updateDraft((current) => ({
                      ...
                      vehicleLookup: {
                        ...current.vehicleLookup,
                        flowId: event.target
                          .value as InsurancePackageSettings["vehicleLookup"]["flowId"],
                      },
                    }))
                  }
                >
