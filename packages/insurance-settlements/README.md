...
Optional independent extension `insurance.settlements`. Register `extension` from `./manifest` (including runtime settings), `requirement` from `./object`, and `screens` from `./admin`. The collection is a navigation anchor; operational state lives in extension settings on the backend.
