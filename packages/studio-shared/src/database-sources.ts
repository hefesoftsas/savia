...
const credentials = {
  username: text,
  password: z.string().max(10000).default(""),
};
...
export const databaseConnectionSchemas = {
  postgres: z
    .object({
      ...
      schema: databaseFieldName.default("public"),
  ...
  mssql: z
    .object({
      kind: z.literal("mssql"),
      ...common,
      ...credentials,
      port: port.default(1433),
      schema: databaseFieldName.default("dbo"),
  ...
  mongodb: z
    .object({
      ...
      authSource: text.default("admin"),
      ssl: z.boolean().default(true),
    })
    .strict(),
};
export const databaseConnectionSchema = z.discriminatedUnion("kind", [
  databaseConnectionSchemas.postgres,
  databaseConnectionSchemas.mysql,
  databaseConnectionSchemas.mssql,
  databaseConnectionSchemas.mongodb,
]);
...
export const databaseSourceInputSchema = z.discriminatedUnion("kind", [
  databaseConnectionSchemas.postgres.extend(source),
  databaseConnectionSchemas.mysql.extend(source),
  databaseConnectionSchemas.mssql.extend(source),
  databaseConnectionSchemas.mongodb.extend(source),
]);
...
export function databaseSourceConfigFromInput(value: unknown) {
  const {
    ...
    kind: _kind,
    ...config
  } = databaseSourceInputSchema.parse(value);
  return config;
}
export const databaseSourceConfigSchema = z
  .unknown()
  .transform((value, ctx) => {
    const parsed = databaseSourceInputSchema.safeParse({
      ...(value as object),
      id: "source",
      label: "Source",
    });
    if (!parsed.success) {
      ctx.addIssue({
        code: "custom",
        message: "Invalid database source configuration",
      });
      return z.NEVER;
    }
...
function safeJson(value: unknown, depth = 0): value is JsonValue {
  ...
  if (Array.isArray(value))
    return value.length <= 1000 && value.every((v) => safeJson(v, depth + 1));
...
export const resourceMetadataSchema = z
  .object({
    ...
    primaryKey: z.array(databaseFieldName).max(16),
    uniqueKeys: z.array(z.array(databaseFieldName).max(16)).max(100),
...
const envelope = {
  ...
  resource: databaseResourceName,
  columns: z.array(databaseFieldName).min(1).max(100),
  idColumn: databaseFieldName.optional(),
  idType: z.enum(["string", "objectId"]).optional(),
};
export const databaseReadSchema = z
  .object({
    ...envelope,
    ...
    sort: databaseFieldName.optional(),
    ...
    searchColumns: z.array(databaseFieldName).max(20).default([]),
  })
  .strict()
  .refine(
    (v) => v.operation !== "read" || Boolean(v.id && v.idColumn),
    "Reading requires an identifier",
  );
export type DatabaseRead = z.infer<typeof databaseReadSchema>;
const values = z
  .record(databaseFieldName, jsonValueSchema)
...
export const databaseMutationSchema = z.discriminatedUnion("operation", [
  z
    .object({ ...mutationEnvelope, operation: z.literal("create"), values })
    .strict(),
  z
    .object({
      ...mutationEnvelope,
      ...
      values: values.refine((v) => Object.keys(v).length > 0),
    })
    .strict(),
  z
    .object({
...
export const databaseResultSchema = z
  .object({
    data: z.union([resultRow, z.array(resultRow), z.null()]),
...
export const databaseResourcesSchema = z
  .array(
    z
      .object({
        resource: databaseResourceName,
  ...
  .max(500);
export function resolveRecordKey(
  meta: ResourceMetadata,
  selected?: string,
): string | undefined {
  if (meta.kind === "collection")
    return meta.idType && (!selected || selected === "_id") ? "_id" : undefined;
  const keys = [meta.primaryKey, ...meta.uniqueKeys]
    .filter(
      (k) =>
        k.length === 1 &&
        meta.fields.some((f) => f.name === k[0] && !f.nullable),
    )
    .map((k) => k[0]!);
  return selected ? keys.find((k) => k === selected) : keys[0];
}
export function deriveDatabaseCapabilities(
  meta: ResourceMetadata,
  writeEnabled: boolean,
  idColumn?: string,
) {
  const key = resolveRecordKey(meta, idColumn);
  const write = writeEnabled && Boolean(key) && meta.kind !== "view";
  return {
    list: true,
    read: Boolean(key),
    create: write,
...
export function databaseFieldInterface(field: DatabaseField) {
  ...
  return {
    ...
    readOnly: field.generated || !field.writable,
    required: !field.nullable && !field.hasDefault && !field.generated,
  };
}
...
export class DatabaseBridgeError extends Error {
  constructor(
    ...
    public readonly status:
      400 | 403 | 404 | 405 | 409 | 422 | 502 | 503 | 504 = 502,
