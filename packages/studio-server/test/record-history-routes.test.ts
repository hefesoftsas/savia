...
>;
const app = createStudioApp("history", { principalId: "alice" });
const request = (
  path: string,
  method = "GET",
  body?: unknown,
...
function scoped(name: string, p: AccessPolicy) {
  return (path: string) =>
    request(
      ...
      "GET",
      undefined,
      createStudioApp("history", { principalId: "alice", accessPolicy: p }),
  ...
  expect(
    (
      await request(
        ...
        "GET",
        undefined,
        createStudioApp("other", { principalId: "bob" }),
      )
    ).status,
  ).toBe(404);
});
...
it("honors the factory access policy even when its caller supplies the raw database", async () => {
  ...
  await enable(name);
  const record = await create(name);
  const selected = createStudioApp("history", {
    principalId: "alice",
...
it("rejects forbidden, missing, expired and truncated restoration sources", async () => {
  ...
  const reader = createStudioApp("history", {
    principalId: "alice",
    accessPolicy: policy(name),
  });
  expect(
