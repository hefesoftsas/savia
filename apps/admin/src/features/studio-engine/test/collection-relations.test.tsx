  ...
  ReactFlow: ({
    ...
    <div>
      {nodes.map((node: any) => {
        ...
        return (
          <div key={node.id}>
            <button onClick={() => onNodeClick(null, node)}>
              Seleccionar {node.data.label}
            </button>
      ...
      <button
        onClick={() =>
          onConnect({
            ...
            target: "clients",
            sourceHandle: "out:id",
            targetHandle: "in:input_agency_id",
          })
        }
      >
        Conectar relación nativa
      </button>
    </div>
  ),
...
const objects = [
  ...
  config: makeConfig(
    o.name === "clients"
      ? { agency_id: { type: "Textbox", label: "Agencia" } }
...
const nativeObjects = [
  ...
  config: makeConfig(
    o.name === "clients"
      ? { input_agency_id: { type: "Textbox", label: "ID de agencia" } }
      : { name: { type: "Textbox", label: "Nombre" } },
  ),
}));
it("prepares connection without saving until explicit form submission", async () => {
  ...
  render(
    ...
    >
      <CollectionRelations objects={objects} />
    </QueryClientProvider>,
  );
  await waitFor(() =>
    expect(
      screen.getByRole("button", { name: "Nueva relación", exact: true }),
    ).toBeEnabled(),
  );
  ...
  expect(screen.getByLabelText("Nombre visto desde el origen")).toHaveValue(
    "Clientes",
  );
  expect(vi.mocked(api).mock.calls.some((c) => c[1] === "POST")).toBe(false);
  fireEvent.change(screen.getByLabelText("Nombre visto desde el destino"), {
    target: { value: "Agencia" },
  });
  fireEvent.click(
    screen.getByRole("button", { name: "Crear relación", exact: true }),
  );
  await waitFor(() =>
    expect(api).toHaveBeenCalledWith("/collection-relations", "POST", {
      sourceObject: "agencies",
      targetObject: "clients",
...
it("renders field handles and edits an existing relation with its version", async () => {
  ...
  fireEvent.change(screen.getByLabelText("Nombre visto desde el origen"), {
    target: { value: "Cartera" },
  });
  fireEvent.click(screen.getByRole("button", { name: "Guardar cambios" }));
  const { id, ...payload } = relation;
  await waitFor(() =>
    expect(api).toHaveBeenCalledWith("/collection-relations/r1", "PUT", {
      ...payload,
      sourceLabel: "Cartera",
    }),
  );
...
it("keeps native physical fields immutable and allows cancelling edits", async () => {
  ...
  expect(screen.getByLabelText("Colección de origen")).toBeDisabled();
  expect(
    screen.getByLabelText("Campo para mostrar el origen"),
  ).not.toBeDisabled();
  fireEvent.click(screen.getByRole("button", { name: "Cancelar edición" }));
  expect(
    screen.queryByLabelText("Colección de origen"),
...
it("opens the native relation when the map uses an exposed field alias", async () => {
  ...
  render(
    ...
    >
      <CollectionRelations objects={nativeObjects} />
    </QueryClientProvider>,
  );

  await screen.findByRole("button", { name: "Editar arista" });
  fireEvent.click(
    await screen.findByRole("button", { name: "Conectar relación nativa" }),
  );
...
it("preserves automatic display names when editing only a native label", async () => {
  ...
  const { id, ...payload } = relation;
  await waitFor(() =>
    expect(api).toHaveBeenCalledWith("/collection-relations/native", "PUT", {
      ...payload,
      sourceLabel: "Cartera",
    }),
  );
...
it("adds an unrelated entity to a focused map without creating a saved relation", async () => {
  ...
  render(
    <QueryClientProvider client={new QueryClient()}>
      <CollectionRelations objects={objects} focusObject="clients" />
    </QueryClientProvider>,
  );
  await waitFor(() =>
    expect(
  ...
  expect(
    screen.queryByRole("button", { name: "Seleccionar Agencias" }),
  ).not.toBeInTheDocument();
  fireEvent.change(screen.getByLabelText("Entidad para agregar al mapa"), {
    target: { value: "agencies" },
  });
  fireEvent.click(
    screen.getByRole("button", { name: "Agregar entidad", exact: true }),
  );
  expect(
    screen.getByRole("button", { name: "Seleccionar Agencias" }),
  ).toBeInTheDocument();
...
it("prepares a supported manual relationship for connected entities", async () => {
  vi.mocked(api).mockResolvedValue({ data: [] });
  const connected = objects.map((o) => ({
    ...o,
    config: {
      ...o.config,
      studio: { ...o.config.studio, business: "managed-customer" as const },
    },
  }));
  render(
    <QueryClientProvider client={new QueryClient()}>
      <CollectionRelations objects={connected} focusObject="clients" />
    </QueryClientProvider>,
  );
  await waitFor(() =>
    expect(
  ...
  fireEvent.change(screen.getByLabelText("Colección de destino"), {
    target: { value: "agencies" },
  });
  expect(screen.getByLabelText("Cómo se relacionan los registros")).toHaveValue(
    "local",
  );
  expect(screen.getByLabelText("Nombre visto desde el origen")).toHaveValue(
    "Agencias",
  );
  expect(screen.getByLabelText("Nombre visto desde el destino")).toHaveValue(
    "Clientes",
  );
  expect(
    screen.getByRole("button", { name: "Crear relación", exact: true }),
...
it("shows a new relation created between entities outside the original focus", async () => {
  ...
  vi.mocked(api).mockImplementation(async (_path, method, body) => {
    if (method === "POST")
      saved = [{ ...(body as object), id: "new", version: 1 }];
    return { data: saved };
  });
  render(
    <QueryClientProvider client={new QueryClient()}>
  ...
  expect(
    screen.getByRole("button", { name: "Seleccionar Otra entidad" }),
  ).toBeInTheDocument();
});
...
it("reopens an existing manual relation when connected fields normalize to IDs", async () => {
  ...
  render(
    <QueryClientProvider client={new QueryClient()}>
      <CollectionRelations objects={connected} />
    </QueryClientProvider>,
  );
  await screen.findByRole("button", { name: "Editar arista" });
  fireEvent.click(screen.getByRole("button", { name: "Conectar mapa" }));
...
it("removes a related entity only from the map and allows adding it back", async () => {
  ...
  await screen.findByRole("button", { name: "Editar arista" });
  fireEvent.click(
    screen.getByRole("button", { name: "Quitar Agencias del mapa" }),
  );
  expect(
    screen.queryByRole("button", { name: "Seleccionar Agencias" }),
  ).not.toBeInTheDocument();
  expect(
    screen.queryByRole("button", { name: "Editar arista" }),
  ).not.toBeInTheDocument();
  expect(vi.mocked(api).mock.calls.some((call) => call[1] === "DELETE")).toBe(
    false,
  );
  expect(
    screen.getByRole("button", { name: "Editar relación", exact: true }),
  ).toBeInTheDocument();
  fireEvent.change(screen.getByLabelText("Entidad para agregar al mapa"), {
    target: { value: "agencies" },
  });
  ...
  expect(
    screen.getByRole("button", { name: "Editar arista" }),
  ).toBeInTheDocument();
});
...
it("publishes a field from the map and exposes it as a connection endpoint", async () => {
  vi.mocked(api).mockImplementation(async (path, method, body) => {
    if (path === "/objects/clients" && method === "PUT")
      return { data: { ...(body as object), name: "clients", version: 2 } };
    return { data: [] };
  });
  render(
    <QueryClientProvider client={new QueryClient()}>
  ...
  fireEvent.click(
    screen.getByRole("button", { name: "Crear campo en Clientes" }),
  );
  fireEvent.change(screen.getByLabelText("Nombre del campo"), {
    target: { value: "Código de agencia" },
  });
  fireEvent.change(screen.getByLabelText("Identificador del campo"), {
    target: { value: "agency_code" },
  });
    ...
    expect.objectContaining({
      ...
      config: expect.objectContaining({
        fields: expect.objectContaining({
          agency_id: expect.any(Object),
          agency_code: { type: "Textbox", label: "Código de agencia" },
        }),
        fieldOrder: ["agency_id", "agency_code"],
      }),
    }),
  );
  fireEvent.click(
  ...
  fireEvent.change(screen.getByLabelText("Campo de destino"), {
    target: { value: "agency_code" },
  });
...
it("keeps a failed field draft and does not display a field that was not saved", async () => {
  ...
  expect(api).toHaveBeenCalledWith(
    ...
    expect.objectContaining({
      config: expect.objectContaining({
        fields: expect.objectContaining({
          code: { type: "Textbox", label: "Código", config: { unique: true } },
        }),
      }),
    }),
  );
...
it("rejects an existing field identifier without overwriting its definition", async () => {
  ...
  fireEvent.change(screen.getByLabelText("Identificador del campo"), {
    target: { value: "agency_id" },
  });
  fireEvent.click(screen.getByRole("button", { name: "Guardar campo" }));
  expect(await screen.findByRole("alert")).toHaveTextContent(
    "Ya existe un campo",
  );
  expect(vi.mocked(api).mock.calls.some((call) => call[1] === "PUT")).toBe(
    false,
  );
});
...
it("binds an existing manual relation to its form field and connects that handle", async () => {
  ...
  vi.mocked(api).mockImplementation(async (path, method, body) => {
    if (path === "/objects/clients" && method === "PUT")
      return { data: { ...(body as object), version: 2 } };
    if (method === "PUT") {
      relation = { ...relation, ...(body as object), version: 2 };
  ...
  fireEvent.click(await screen.findByRole("button", { name: "Editar arista" }));
  fireEvent.change(screen.getByLabelText("Campo del formulario de destino"), {
    target: { value: "agency_id" },
  });
  fireEvent.click(screen.getByRole("button", { name: "Guardar cambios" }));
  await waitFor(() =>
  ...
  expect(api).toHaveBeenCalledWith(
    ...
    expect.objectContaining({
      ...
      config: expect.objectContaining({
        fields: expect.objectContaining({
          agency_id: expect.objectContaining({
            config: expect.objectContaining({
  ...
  expect(screen.getByRole("button", { name: "Editar arista" })).toHaveAttribute(
    "data-target-handle",
    "in:agency_id",
  );
});
...
it("allows rebinding a field whose former relation no longer exists", async () => {
          ...
          config: {
            ...
            fields: {
              agency_id: {
                type: "Textbox",
                label: "Agencia",
                config: { collectionRelation: "deleted" },
              },
            },
          },
        }
  ...
  render(
    <QueryClientProvider client={new QueryClient()}>
      <CollectionRelations objects={stale} />
    </QueryClientProvider>,
  );
  await waitFor(() =>
    expect(
  ...
  fireEvent.change(screen.getByLabelText("Cómo se relacionan los registros"), {
    target: { value: "local" },
  });
  fireEvent.change(screen.getByLabelText("Campo del formulario de destino"), {
    target: { value: "agency_id" },
  });
  expect(screen.getByLabelText("Campo del formulario de destino")).toHaveValue(
    "agency_id",
  );
});
...
it("renders create field and remove node actions as icon buttons with tooltips", async () => {
  ...
  const createButton = screen.getByRole("button", {
    name: "Crear campo en Clientes",
  });
  const removeButton = screen.getByRole("button", {
    name: "Quitar Clientes del mapa",
  });
