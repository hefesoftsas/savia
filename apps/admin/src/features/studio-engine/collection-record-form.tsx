...
function ManualRelationsForm(props: DynamicFormProps) {
  ...
  useEffect(() => {
    ...
    const baseline: DraftSnapshot = {
      values,
      previous: recordId ? (props.values as StudioRecord) : undefined,
      previousIds: initial,
  ...
  async function discardDraft() {
    ...
    try {
      ...
      if (recordId) {
        ...
        const response = await api<{ data: StudioRecord }>(
  ...
  async function save(data: Record<string, unknown>, previous?: StudioRecord) {
    if (!initialized)
      throw new Error(
