...
const label = (record: StudioRecord) =>
  String(
    record.name ??
      record.display_name ??
...
export default function RecordDetail({
  ...
  object: StudioObject;
  record: StudioRecord;
  onEdit: (record: StudioRecord) => void;
  onDuplicate?: (record: StudioRecord) => void;
  onClose: () => void;
  onRefresh: () => void;
  onQuotation?: (record: StudioRecord) => void;
  ...
  const detail = useQuery({
    ...
    queryFn: () =>
      api<{ data: { record: StudioRecord; relations: Relation[] } }>(
  ...
  async function upload(file?: File) {
    ...
    try {
      ...
      const response = await studioFetch(`/api/files/${base}`, {
        method: "POST",
        body: form,
      });
        ...
        <DialogHeader>
          <div className="op-section-title">
            <div>
              ...
              <DialogTitle>
                {studioTitle ? display(studioTitle, uiLocale) : label(current)}
              </DialogTitle>
            </div>
