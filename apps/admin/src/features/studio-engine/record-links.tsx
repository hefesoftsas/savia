...
function recordLabel(record: StudioRecord) {
  ...
  const fallback = Object.entries(record).find(
    ([key, value]) =>
      !["id", "created_at", "updated_at", "_version"].includes(key) &&
...
function cellValue(value: unknown): string {
  ...
  if (Array.isArray(value))
    return value.map((item) => cellValue(item)).join(", ");
    ...
    <div className="records-list related-records-list">
      ...
      >
        <div
          ...
          className="records-table related-records-table"
        >
          <table
            aria-label={t("Registros relacionados: %{p0}", { p0: group.label })}
          >
            <thead>
              <tr>
                ...
                {group.canEdit && <th scope="col">{t("Acciones")}</th>}
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  <tr key={row.id}>
                    {visibleColumns.map((column, index) => {
                      const raw =
                        column.key === "label" ? row.label : data[column.key];
                      const value = cellValue(
                        raw ?? (index === 0 ? row.label : undefined),
                      );
                      return (
                        <td
                          key={column.key}
                        ...
                        >
                          {index === 0 && !row.missing ? (
                            <button
                              ...
                              aria-label={value}
                              onClick={() =>
                                onNavigate(group.targetObject, row.id)
                              }
                            >
                              {value}
                            </button>
                    ...
                    {group.canEdit && (
                      <td className="records-table-actions-column">
                        <Button
                          ...
                          variant="ghost"
                          disabled={busy}
                          aria-label={t("Desvincular %{p0}", { p0: row.label })}
...
function LinkGroup({
  ...
  const candidates = useQuery({
    ...
    queryFn: () =>
      api<{ data: StudioRecord[]; total: number }>(
        `/records/${enc(group.targetObject)}?page=${page}&perPage=20`,
      ),
    enabled: selecting && group.canEdit,
  });
  async function mutate(method: string, id: string) {
    setBusy(true);
    setError("");
    try {
      await api(`${base}/${enc(group.definition.id)}`, method, {
        targetId: id,
      });
      setTarget("");
      ...
      setError(
        e instanceof Error
          ? e.message
          : t("No se pudo actualizar la asociación. Intenta nuevamente."),
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <section className="min-w-0 space-y-3 border-b pb-4">
      {group.canEdit && (
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          >
            {selecting ? t("Cerrar selector") : t("Vincular registro")}
          </Button>
        </div>
      )}
      {group.cardinalityConflict && (
      ...
      {group.records.length === 0 && (
        <p className="text-sm text-muted-foreground">
          {t("Sin registros vinculados en esta página.")}
        </p>
      )}
      {group.records.length > 0 && (
        <RelatedRecordsTable
      ...
      {selecting && (
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            if (targetId) void mutate("POST", targetId);
          }}
        >
          <Label
            htmlFor={`link-target-${group.definition.id}-${group.direction}`}
          >
            {t("Registro de")} {group.targetLabel}
          </Label>
          ...
          <select
            id={`link-target-${group.definition.id}-${group.direction}`}
          ...
          >
            <option value="">{t("Seleccionar registro")}</option>
            {candidates.data?.data.map((r) => (
          ...
          {candidates.data?.data.length === 0 && (
            <p className="text-sm">
              {t("No hay registros disponibles en esta página.")}
            </p>
          )}
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button
              ...
              disabled={
                ...
                !candidates.data ||
                (candidates.data.total !== undefined
                  ? page * 20 >= candidates.data.total
                  : candidates.data.data.length < 20)
              }
              onClick={() => {
                setPage(page + 1);
                setTarget("");
            ...
            <Button
              size="sm"
              disabled={!targetId || busy || candidates.isFetching}
            >
              {busy ? t("Guardando…") : t("Vincular")}
            </Button>
          </div>
          <p className="text-xs text-muted-foreground">
            {t(
...
function RecordLinksContent({
  ...
  showGroupTabs,
  showHeading,
}: RecordLinksProps & { showGroupTabs: boolean; showHeading: boolean }) {
  const t = useMessages(recordsMessages);
  const locale = useAppLocale();
  ...
  const activeGroup =
    groups?.find((group) => relationKey(group) === selectedGroupKey) ??
    groups?.[0];
  const tabScroll = useHorizontalScroll(
    groups?.map((group) => relationKey(group)).join("|") ?? "",
  );
  return (
    <div className="record-links-tabs">
      {showHeading && (
        <h3 className="font-medium">{t("Registros relacionados")}</h3>
      )}
      {query.isPending && (
        <div className="space-y-3" role="status" aria-live="polite">
          <p className="text-xs text-muted-foreground">
            {t("Cargando relaciones…")}
          </p>
      ...
      {showGroupTabs && groups?.length ? (
        <div className="record-links-tablist-shell">
          <div
            ...
            role="tablist"
            aria-label={t("Tipos de registros relacionados")}
          >
            {groups.map((group) => {
