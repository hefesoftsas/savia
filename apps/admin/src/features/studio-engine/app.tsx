...
import { inspectDocument } from "@savia/studio-shared/openapi";
import "./style.css";
...
function App({
  ...
  const [mobile, setMobile] = useState(false),
    ...
    [editingState, setEditing] = useState<StudioRecord | "new" | null>(null);
  ...
  const editing =
    view === "create"
      ...
      : view === "edit"
        ...
        : editingState;
  const openRecord = (record: StudioRecord) => {
  ...
  const editRecord = (record: StudioRecord | "new") => {
    if (
      !object ||
      !(record === "new"
  ...
  const duplicateRecord = (record: StudioRecord) => {
  ...
  const updateScreenVisibility = async (
    target: StudioObject,
    hidden: boolean,
  ) => {
    try {
      ...
      const result = await api<{ data: StudioObject }>(
  ...
  useEffect(() => {
    ...
    window.addEventListener("savia-studio-objects-changed", refreshObjects);
    return () =>
      window.removeEventListener(
        "savia-studio-objects-changed",
        refreshObjects,
      );
                        ...
                        activeQueryClient.setQueryData(
                          ...
                          (previous: any) =>
                            previous
                              ? {
                                  ...previous,
                                  data: previous.data.map((o: StudioObject) =>
                                    o.name === updated.name
                                      ? { ...o, ...updated }
                                      : o,
                                  ),
