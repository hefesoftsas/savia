...
describe("generated request page runtime", () => {
  ...
  it('runs a field lookup without submitting the quote and rejects stale responses', async()=>{
    ...
    page.config.studio!.requestPage!.actions.push({id:'lookup',operationId:'execute_lookup',label:'Consultar placa',kind:'lookup',input:{plate:'plate'},output:{year:'/data/vehicle/year'}});
    ...
    setCrmRuntime({embedded:true,domainId:'platform',requestTransport:transport});
    render(<RequestPage object={page}/>);
    await screen.findByText('Al ejecutar una operación, sus resultados aparecerán aquí.');
    await user.type(screen.getByRole('textbox',{name:/Placa/}),'TESTCAR');
    ...
    resolveRun(Response.json({id:'lookup-run',actionId:'lookup',label:'Consulta',mode:'mock',status:'complete',createdAt:new Date().toISOString(),values:{plate:'TESTCAR'},result:{type:'vehicle_lookup',status:'success',data:{vehicle:{plate:'TESTCAR',year:2024}},errors:[],warnings:[],metadata:{simulated:true}}}));
    await screen.findByText('Cambió la entrada; consulta de nuevo para completar los campos.');
    expect(screen.getByRole('spinbutton',{name:'Año'})).not.toHaveValue(2024);
    ...
    await user.click(screen.getByRole('button', {name:'Ejecutar'}));
    await waitFor(() => expect(transport.mock.calls.filter(([,init])=>init?.method==='POST')).toHaveLength(2));
    const posts = transport.mock.calls.filter(([,init])=>init?.method==='POST');
    expect(JSON.parse(String(posts[1][1]?.body)).actionId).toBe('quote');
    resolveRun(Response.json({id:'quote-run',actionId:'quote',label:'Aseguradora',mode:'mock',status:'failed',createdAt:new Date().toISOString(),values:{plate:'TESTCAR'},result:null,error:'Test'}));
    await waitFor(() => expect(screen.getByRole('button', {name:'Ejecutar'})).toBeEnabled());
  });

  it.each(['steps', 'conversation'] as const)('keeps lookup results while navigating a %s wizard without quoting early', async (presentation) => {
    const user = userEvent.setup();
    const page = structuredClone(object);
    page.config.fields.plate.config = {step:'vehicle'};
    page.config.fields.year = {type:'Number',label:'Año',required:true,config:{step:'details'}};
    page.config.fieldOrder!.push('year');
    page.config.studio!.wizard = {enabled:true,presentation,steps:[{id:'vehicle',title:'Vehículo'},{id:'details',title:'Detalles'}]};
    page.config.studio!.requestPage!.actions.push({id:'lookup',operationId:'lookup',label:'Consultar placa',kind:'lookup',input:{plate:'plate'},output:{year:'/data/vehicle/year'}});
    const transport = vi.fn(async (_path:string, init?:RequestInit) => {
      if(init?.method !== 'POST') return Response.json({data:[]});
      const body = JSON.parse(String(init.body));
      return Response.json({...body,label:'Consulta',status:'complete',createdAt:new Date().toISOString(),result:{type:'vehicle_lookup',status:'success',data:{vehicle:{plate:'REDACTD',year:2024}},errors:[],warnings:[],metadata:{simulated:true}}});
    });
    setCrmRuntime({embedded:true,domainId:'platform',requestTransport:transport});
    render(<RequestPage object={page}/>);
    await user.type(screen.getByRole('textbox',{name:/Placa/}),'REDACTD');
    expect(screen.queryByRole('region',{name:'Resultados de la página'})).not.toBeInTheDocument();
    await user.click(screen.getByRole('button',{name:'Resultados'}));
    expect(screen.getByRole('region',{name:'Resultados de la página'})).toBeVisible();
    await user.click(screen.getByRole('button',{name:'Preparar cotización'}));
    expect(screen.getByRole('textbox',{name:/Placa/})).toHaveValue('REDACTD');

    await user.click(screen.getByRole('button',{name:'Consultar placa'}));
    await waitFor(()=>expect(screen.getByRole('button',{name:'Consultar placa'})).toBeEnabled());
    await user.click(screen.getByRole('button',{name:presentation==='steps'?'Siguiente paso':/Continuar/}));
    await waitFor(()=>expect(screen.getByRole('spinbutton',{name:'Año'})).toHaveValue(2024));
    expect(transport.mock.calls.filter(([,init])=>init?.method==='POST')).toHaveLength(1);
  });
