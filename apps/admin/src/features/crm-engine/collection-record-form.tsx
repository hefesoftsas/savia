...
import type { RelationDefinition, RecordRelationGroup } from "@savia/crm-shared/relations";
...
export default function CollectionRecordForm(props: DynamicFormProps) {
  ...
  if (!runtime.apiBasePath?.startsWith("/v1/data-domains/")) return <DynamicForm {...props} />;
  return <ManualRelationsForm key={`${runtime.apiBasePath}:${props.object.name}:${props.values?.id ?? "new"}`} {...props} />;
}
