...
function SidebarProvider({
  ...
  const setWidth = React.useCallback((nextWidth: number) => {
    const clamped = Math.min(
      SIDEBAR_WIDTH_MAX_PX,
      Math.max(SIDEBAR_WIDTH_MIN_PX, nextWidth),
    );
...
const SIDEBAR_RESIZE_HANDLE_WIDTH_PX = 20;
...
function SidebarResizeHandle({
  ...
  return (
    <div
      ...
      className={cn(
        "fixed inset-y-0 z-50 hidden cursor-col-resize touch-none select-none md:block",
        "after:absolute after:inset-y-0 after:left-1/2 after:w-0.5 after:-translate-x-1/2 after:rounded-full after:bg-border/50 after:transition-colors",
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn(
        "h-8 w-full bg-transparent shadow-none dark:bg-transparent",
        className,
      )}
      {...props}
    />
