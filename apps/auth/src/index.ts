...
export default {
  async fetch(request: Request, environment: AuthWorkerEnvironment) {
    ...
    if (user) {
      ...
      if (request.method === "PATCH") return updateUser(auth, request, userId);
      if (request.method === "DELETE") return removeUser(auth, request, userId);
    }
    return Response.json(
