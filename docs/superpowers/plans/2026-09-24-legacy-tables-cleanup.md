...
**Goal:** Remove 190 of 199 legacy Django tables (phase 1: 109 with zero references; phase 2: 81 more after retiring the legacy import scripts). Keep 9 legacy tables: the 5 live sync tables plus their 4 FK parents. Shrink D1 333 → 143 tables with Postgres in parity, without breaking replay, sync, or tests.

**Context:** `packages/db/migrations/0006_full_source_schema.sql` projected 199 legacy tables into D1 (251 KB, 4.582 lines) with a matching `packages/db/src/internal-schema.generated.ts` (3.161 lines, `internalTables` with zero importers) and `packages/db/postgres/0001_baseline.sql` (4.412 lines). The system runs on raw SQL via `dialectFor` (`studio_*`, `workflows`, `tenants`, `identity_*`, `crm_*`) plus 5 live `customer_*` tables. A full-repo audit (runtime SQL, post-0006 triggers/views/FKs, tests, `scripts/*.mjs`) plus FK-closure analysis set the final tally: drop 190, retain 9. `apps/api/test/schema.test.ts` froze the total at 333, then 224, now 143.

**Phase 3 note — attempted and ABANDONED (2026-09-24):** removing the 4 FK parents (`customer_client`, `customer_group`, `business_commercialunit`, `app_economicactivity`) by rebuilding the 5 live tables proved undeployable and was reverted; the 9-table state stands. What was learned, kept as record: (1) SQLite records deferred FK violations at `DROP` time and only clears them on a _later_ parent-key insert, so drop+rename+insert-before-drop fails at `COMMIT` — proven by minimal repro; only stage-out/copy-back converges in one transaction. (2) `RENAME TO` fails while any trigger references the transiently-missing name. (3) Decisively: D1's deploy path (`scripts/apply-d1-migrations.mjs`) sends **one statement per request**, each in its own implicit transaction, so `PRAGMA defer_foreign_keys` cannot cover multi-statement rebuilds — a preview deploy failed at `0071` statement 51/80 (`DROP TABLE customer_clientagency` with live referencing rows) and rolled back. Removing constraints from data-referenced tables is impossible on that path without evacuating live rows, which was rejected as unjustifiable risk for 4 tiny dimension tables. The retired `0071` rebuild/`0013` never completed anywhere; `0071` now contains only an idempotent repair (drop staging leftovers, restore pre-existing trigger sets, restore one FK clause lost on the preview database by the partial run) that converges fresh and partial databases to identical schema. Native `0013` was deleted before ever applying.

**Architecture:** Additive, immutable-migration style. `0006` is never edited. `0069_drop_unused_legacy_tables.sql` dropped the first 109 dead tables and `0070_drop_remaining_legacy_tables.sql` drops 81 more (`DROP TABLE IF EXISTS`, children-first topological order; the single self-reference cycle `renewal_renewal -> renewal_renewal` needs no ordering). Both run after `0046` (whose DML touches dropped tables while they still exist). `0071` is an idempotent repair converging every database to identical schema (see phase-3 note). The generated Drizzle mirror is trimmed by script to the final 9. Postgres gets matching `0011`/`0012` drops. Tests and manifests track the inventory (333 → 224 → 143 D1 tables).

**Tech Stack:** TypeScript, SQLite/D1, PostgreSQL, Drizzle (generated mirror only), Vitest, Node contract tests. No new dependencies.

**FK closure (2026-09-24 verification):** phase 1 kept 8 extra tables as FK parents; phase 2 recomputed the closure around the 5 sync tables and found only 4 true parents must stay (`customer_client`, `customer_group`, `business_commercialunit`, `app_economicactivity`). Final tally: drop 190, retain 9. Full replay of migrations `0000`–`0071` in scratch SQLite with `PRAGMA foreign_keys=ON` passes: 143 tables, 55 triggers intact, 0 FK violations.
...
**Retain (9 legacy):** the 5 live sync tables (`customer_clientagency`, `customer_naturalperson`, `customer_legalperson`, `customer_address`, `customer_legalpersoncontact`) plus their 4 FK parents (`customer_client`, `customer_group`, `business_commercialunit`, `app_economicactivity`), which have no runtime readers and exist only for constraint integrity (see phase-3 note for why they stay).
...
- [x] Task 1 — D1 migrations `0069` (109 drops) + `0070` (81 drops) + `0071` (idempotent repair converging schema+triggers).
- [x] Task 2 — Trim `internal-schema.generated.ts` to the final 9 (script-filtered, header kept).
- [x] Task 3 — Update `apps/api/test/schema.test.ts` count 333 → 224 → 143; dropped-table assertions removed/repointed at `customer_clientagency`.
- [x] Task 4 — Postgres `0011` + `0012` (both apply cleanly on live PG16) + manifests: 80 sourceMigrations, tables 325 → 135, objects pruned (0 triggers touched). Checksum-pin test green.
- [x] Task 5 — Refresh `docs/full-d1-schema-manifest.json` (tables 209 → 100 → 19) and retire the 3 legacy import scripts + tests with their `package.json` lint/contracts entries.
- [x] Task 6 — Verify: `pnpm run typecheck` clean; `apps/api` 547/547; `studio-server` 213/213; contracts 60/60; migration contract 7/7; D1 replay `0000→0071` with FKs on (143 tables, 0 violations, incl. partial-preview convergence simulation); PG `0011`/`0012` apply clean standalone with manifest parity; checksum-pin green. Pre-existing failures identical on main: 2 admin UI timing tests, 1 savia-request suite timing test, 9 PG-live tests (env issue in untouched `0005`).
...
2. `0046` runs before the drops, so its DML still sees every table at replay (verified in scratch replay).
