...
The private bootstrap registers the service worker before loading the application modules. Cold start shows a branded splash (logo with a rotating ring) from first paint: a static version is inlined in `index.html` and React takes over with the same `PwaSplash` while the lazy application modules load. Returning to a visible tab or reconnecting checks for a new worker, at most once per minute and without polling. A replacement controller shows an update notice; it never reloads an open form automatically.
