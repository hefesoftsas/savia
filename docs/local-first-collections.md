...
The private bootstrap registers the service worker before loading the application modules. Cold start shows a branded splash (logo with a rotating ring) from first paint: a static version is inlined in `index.html` and React takes over with the same `PwaSplash` while the lazy application modules load, including as the admin loading page during the auth check, so opening the app shows a single continuous visual. Returning to a visible tab or reconnecting checks for a new worker, at most once per minute and without polling. A replacement controller shows an update notice; it never reloads an open form automatically.

If a deployment removes an old lazy-loaded JavaScript or stylesheet asset, both the boot boundary and the administrative error boundary offer **Actualizar y recargar** instead of retrying the same rejected module import. This recovery code is part of the statically imported boot shell. On explicit confirmation it checks the worker script without the HTTP cache, waits for the replacement worker to activate, then reloads the complete document so module failures are no longer cached in memory. While the update installs, the button is disabled and shows a spinner next to **Buscando la nueva versión…**. Offline, failed pre-update checks or a deployment not yet propagated remain retryable; a stalled or failed install after the server already answered drops worker control and reloads from the network instead of stranding the user. The overall update deadline is 30 seconds, with no automatic reload loop.
