...
Normal icons fill 90% of the canvas with the four-leaf mark. A white background
keeps the dark leaves visible in light and dark browser chrome. The SVG favicon
stays crisp at small sizes; the ICO contains 16, 32 and 48 pixel fallbacks.

Maskable icons use a separate composition: the complete mark stays inside the
central safe circle (80% diameter), while the opaque background extends to the
edges for launcher cropping. Never label the normal icon as maskable.
