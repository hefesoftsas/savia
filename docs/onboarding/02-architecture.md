...
## Optional native deployment

The [self-hosted Docker alternative](../guides/self-hosted-docker.md) composes the same Fetch applications with native SQLite, S3, WebSocket, scheduler and isolated-hook adapters. Cloudflare entrypoints continue to use their platform bindings. Business rules, authentication, permissions and local-first synchronization are shared.
